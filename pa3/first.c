...
int main(int argc, char* argv[]) {
    ...
    unsigned setBits = (unsigned)log2((double)setsNum); // Fully associative cache has 0 set bit: if setsNum=1, 2^0 = 1, setBits=0.
    unsigned setAndOffsetBits = setBits + offsetBits;
    int tagBits = BITS - setAndOffsetBits;
    ...
    NWCache nwCache = initNWCache(setsNum, assoc ? assoc : blocksNum);
    ...
    while (fscanf(file, "%c\t%s\n", &c, hexAddress) != EOF && (c == 'R' || c == 'W')) {
        ...
        unsigned long long tag = decAddress >> setAndOffsetBits;
